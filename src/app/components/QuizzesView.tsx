...
export const QuizzesView: React.FC<QuizzesViewProps> = ({ settings,  onNavigate, onSaveQuizHistory }) => {
  ...
  if (activeQuiz) {
    return (
      <div id="active-quiz-container" className="w-full max-w-5xl mx-auto px-4 py-8 md:px-6 md:py-10 min-h-screen bg-[#DBDBE5]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-[#DBDBE5]/60 pb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-[#21222D]/50">{t.activeSession || 'ACTIVE QUIZ'}</span>
  ...
  return (
    <div id="quizzes-view-container" className="w-full max-w-7xl mx-auto px-4 py-6 md:px-8 md:py-10 min-h-screen bg-[#DBDBE5]">
      ...
      <header id="quizzes-view-header" className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-10">
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          ...
          <button 
            ...
            className="w-11 h-11 bg-white border border-[#DBDBE5] text-[#21222D] flex items-center justify-center rounded-full hover:bg-[#DBDBE5]/20 transition-all cursor-pointer shadow-xs relative"
            title="Notifications"
          >
            <Bell className="w-4.5 h-4.5" />
            <span className="absolute top-1 end-1 w-2.5 h-2.5 bg-[#ACD1FD] rounded-full ring-2 ring-white" />
          </button>
          <div 
            id="header-avatar-circle"
            className="w-11 h-11 bg-[#21222D] text-[#DBDBE5] border border-[#21222D] flex items-center justify-center rounded-full font-black text-sm select-none shadow-xs"
          ...
          return (
            <button
              ...
              className={`px-5 py-2.5 rounded-2xl text-xs font-black tracking-wide whitespace-nowrap transition-all cursor-pointer ${
                isSelected 
                  ? 'bg-[#ACD1FD] text-[#21222D] shadow-xs scale-[1.02]' 
                  : 'bg-white border border-[#DBDBE5]/60 text-[#21222D]/70 hover:bg-[#DBDBE5]/15'
              }`}
            >
              {label}
            </button>
      ...
      <section id="recommended-quiz-section" className="mb-12">
        <div className="bg-[#ACD1FD]/10 border border-[#ACD1FD]/30 p-6 md:p-8 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden shadow-xs hover:border-[#ACD1FD]/60 transition-all">
          <div className="space-y-4 max-w-xl z-10">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold text-[#21222D]/70">
              <span className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-full border border-[#DBDBE5]/40">
                <BookText className="w-4 h-4 text-[#21222D]/50" /> 
                20 {t.questionsCountText || 'questions'}
              </span>
              <span className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-full border border-[#DBDBE5]/40">
                <Brain className="w-4 h-4 text-[#21222D]/50" /> 
                B1 {t.levelLabel || 'Level'}
              </span>
              <span className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-full border border-[#DBDBE5]/40">
                <Clock className="w-4 h-4 text-[#21222D]/50" /> 
                ~8 min
              </span>
              <span className="bg-[#ACD1FD] text-[#21222D] px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase">
          ...
          <button 
            ...
            className="px-6 py-4 bg-[#ACD1FD] text-[#21222D] border border-[#21222D]/15 hover:border-[#21222D] font-black text-sm rounded-2xl flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-98 shrink-0 hover:shadow-xs self-start md:self-auto z-10"
          >
      ...
      <section id="browse-by-category-section" className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-black text-[#21222D]">{t.browseByCategory || 'Browse by category'}</h2>
          <button 
            onClick={() => setSelectedCategory('All Quizzes')}
            className="inline-flex items-center gap-1 text-xs font-black text-[#21222D] hover:underline"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { id: 'Vocabulary', label: t.vocabularyCat || 'Vocabulary', count: `24 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A1 - C2', gradient: 'from-[#ACD1FD]/10', border: 'border-[#ACD1FD]/30', hover: 'hover:border-[#ACD1FD]/80', icon: BookOpen, iconCol: 'text-emerald-700 bg-[#ACD1FD]/40' },
            { id: 'Grammar', label: t.grammarCat || 'Grammar', count: `18 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A1 - C2', gradient: 'from-[#958CE8]/10', border: 'border-[#958CE8]/30', hover: 'hover:border-[#958CE8]/80', icon: Pencil, iconCol: 'text-purple-700 bg-[#958CE8]/40' },
            { id: 'Listening', label: t.listeningCat || 'Listening', count: `16 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A2 - C1', gradient: 'from-[#ACD1FD]/10', border: 'border-[#ACD1FD]/30', hover: 'hover:border-[#ACD1FD]/80', icon: Headphones, iconCol: 'text-emerald-700 bg-[#ACD1FD]/40' },
            { id: 'Reading', label: t.readingCat || 'Reading', count: `12 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A2 - C1', gradient: 'from-[#958CE8]/10', border: 'border-[#958CE8]/30', hover: 'hover:border-[#958CE8]/80', icon: BookText, iconCol: 'text-purple-700 bg-[#958CE8]/40' },
            { id: 'Translation', label: t.translationCat || 'Translation', count: `14 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A1 - C2', gradient: 'from-[#ACD1FD]/10', border: 'border-[#ACD1FD]/30', hover: 'hover:border-[#ACD1FD]/80', icon: Languages, iconCol: 'text-emerald-700 bg-[#ACD1FD]/40' },
            { id: 'Mixed', label: t.mixedCat || 'Mixed', count: `20 ${t.quizzesTitle?.toLowerCase() || 'quizzes'}`, level: 'A1 - C2', gradient: 'from-[#958CE8]/10', border: 'border-[#958CE8]/30', hover: 'hover:border-[#958CE8]/80', icon: Sparkles, iconCol: 'text-purple-700 bg-[#958CE8]/40' },
          ].map((cat) => {
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1 mt-auto">
                  <h3 className="font-black text-[#21222D] text-sm md:text-base tracking-tight leading-none mb-1 group-hover:translate-x-0.5 transition-transform">
                    {cat.label}
                  </h3>
                  <p className="text-xs font-semibold text-[#545565] leading-none mb-0.5">{cat.count}</p>
                  <p className="text-[10px] font-black text-[#21222D]/40 leading-none">{cat.level}</p>
                </div>

                {/* Tiny bottom-right circular button */}
                <div className="absolute bottom-4 end-4 w-7 h-7 bg-stone-100 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border border-[#DBDBE5]/40">
                  <ChevronRight className="w-3.5 h-3.5 text-[#21222D]" />
                </div>
              </button>
            );
          })}
      ...
      <div id="bottom-quizzes-cols" className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div id="continue-section-col" className="lg:col-span-7 space-y-4">
          <h2 className="text-xl font-black text-[#21222D]">Continue where you left off</h2>
          
          <div className="bg-white border border-[#DBDBE5]/60 p-5 rounded-3xl flex flex-col sm:flex-row gap-5 items-stretch relative group hover:border-[#21222D]/40 hover:shadow-xs transition-all">
            ...
            <div className="flex-1 flex flex-col justify-between py-1 relative">
              <div className="absolute end-0 top-0 text-[#21222D]/40 cursor-pointer hover:text-[#21222D]">
                <MoreVertical className="w-5 h-5" />
              </div>

              <div>
                <h3 className="text-lg font-black text-[#21222D] mb-3">Travel Vocabulary</h3>
                
                {/* Custom Progress Bar */}
                <div className="space-y-1 mb-4">
                  <div className="flex justify-between text-[11px] font-bold text-[#545565]">
                    <span>Progress</span>
                    <span className="font-extrabold text-[#3b9d4e]">12 / 20 questions</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#DBDBE5] rounded-full overflow-hidden">
                    <div className="h-full bg-[#ACD1FD] rounded-full w-[60%]" />
                  </div>
                </div>

                {/* Badge tags Row */}
                <div className="flex flex-wrap gap-2">
                  <span className="text-[10px] font-bold bg-[#DBDBE5] text-[#21222D]/70 px-2.5 py-1 rounded-full border border-[#DBDBE5]/30">
                    Vocabulary
                  </span>
                  <span className="text-[10px] font-bold bg-[#DBDBE5] text-[#21222D]/70 px-2.5 py-1 rounded-full border border-[#DBDBE5]/30">
                    B1
              ...
              <button 
                ...
                className="px-5 py-2.5 bg-[#ACD1FD] text-[#21222D] border border-[#21222D]/10 hover:border-[#21222D] font-black text-xs rounded-xl flex items-center justify-center gap-1 cursor-pointer transition-all active:scale-95 self-end mt-4 sm:mt-0"
              >
                <span>Continue</span>
        ...
        <div id="recent-completed-col" className="lg:col-span-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black text-[#21222D]">Recently completed</h2>
            <button 
              onClick={() => setSelectedCategory('All Quizzes')}
              className="inline-flex items-center gap-1 text-xs font-black text-[#21222D] hover:underline"
            >
          ...
          <div className="bg-white border border-[#DBDBE5]/60 rounded-3xl p-5 divide-y divide-[#DBDBE5]/30 shadow-xs space-y-3">
            ...
            >
              <div className="flex items-center gap-3.5">
                {/* Circle Plane Icon */}
                <div className="w-10 h-10 rounded-full bg-[#ACD1FD]/25 border border-[#ACD1FD]/45 text-emerald-800 flex items-center justify-center shrink-0">
                  <Plane className="w-4.5 h-4.5" />
                </div>
                <div>
                  <h4 className="font-black text-[#21222D] text-sm group-hover:text-[#3b9d4e] transition-colors">
                    Airport & Travel
                  </h4>
                  <p className="text-[11px] font-bold text-[#545565]">20 questions • Vocabulary</p>
                </div>
              </div>

              <div className="flex items-center gap-3 text-end">
                <div>
                  <div className="font-black text-[#21222D] text-sm">18 / 20</div>
                  <div className="text-[10px] font-black text-[#3b9d4e]">90%</div>
                </div>
                <ChevronRight className="w-4 h-4 text-[#21222D]/30 group-hover:translate-x-0.5 transition-transform" />
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-3.5">
                {/* Circle Message Icon */}
                <div className="w-10 h-10 rounded-full bg-[#958CE8]/25 border border-[#958CE8]/45 text-purple-800 flex items-center justify-center shrink-0">
                  <MessageSquare className="w-4.5 h-4.5" />
                </div>
                <div>
                  <h4 className="font-black text-[#21222D] text-sm group-hover:text-purple-700 transition-colors">
                    Past Simple Tense
                  </h4>
                  <p className="text-[11px] font-bold text-[#545565]">15 questions • Grammar</p>
                </div>
              </div>

              <div className="flex items-center gap-3 text-end">
                <div>
                  <div className="font-black text-[#21222D] text-sm">12 / 15</div>
      ...
      {selectedCategory !== 'All Quizzes' && (
        <section id="filtered-available-quizzes" className="mt-12 pt-8 border-t border-[#DBDBE5]/45">
          <div className="mb-6">
            <h3 className="text-xl font-black text-[#21222D]">
              Quizzes in {selectedCategory}
            </h3>
            <p className="text-xs font-bold text-[#545565] mt-1">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {filteredQuizzes.map((quiz) => (
              <div 
                ...
                key={quiz.id} 
                className="bg-white border border-[#DBDBE5]/60 p-6 rounded-3xl flex flex-col justify-between hover:border-[#21222D]/50 transition-all group shadow-2xs"
              >
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between items-start gap-4">
                    <h4 className="text-lg font-black text-[#21222D] group-hover:text-[#3b9d4e] transition-colors leading-tight">
                      {quiz.title}
                    </h4>
                    <span className="text-[10px] font-black bg-[#DBDBE5] text-[#21222D] px-2.5 py-1 rounded-full border border-[#DBDBE5]/30 uppercase tracking-wider shrink-0">
                      {quiz.level}
                    </span>
                  </div>
                  <p className="text-xs md:text-sm font-semibold text-[#545565] leading-relaxed line-clamp-2">
                    {quiz.description}
                  </p>
                  <div className="inline-block text-[9px] font-black uppercase bg-[#DBDBE5] text-[#21222D] px-2.5 py-1 rounded-full border border-[#DBDBE5]/30">
                    {quiz.category}
                  </div>
                </div>
                
                <div className="flex items-center justify-between pt-4 border-t border-[#DBDBE5]/25">
                  ...
                  <button 
                    ...
                    className="px-4.5 py-2.5 bg-[#21222D] text-[#DBDBE5] border border-[#21222D] hover:bg-stone-800 rounded-xl font-black text-xs transition-all cursor-pointer active:scale-95"
                  >
                    Start
                  </button>
                </div>
